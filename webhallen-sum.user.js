...
(function () {
  ...
  async function fetchOrders(whId) {
    ...
    while (true) {
      ...
      const data = await fetchAPI(
        `https://www.webhallen.com/api/order/user/${encodeURIComponent(
          whId
        )}?filters[history]=true&sort=orderStatus`,
        params
      );
  ...
  async function fetchAchievements(whId) {
    const data = await fetchAPI(
      `https://www.webhallen.com/api/user/${encodeURIComponent(
        whId
      )}/achievements`
    );
    return data;
  ...
  function getOrderDatesPerMonthWithSumKillstreak(orders) {
    const groupedData = Object.entries(
      orders.reduce((acc, { orderDate, sentDate, totalSum }) => {
        ...
        const orderKey =
          new Date(Date.UTC(orderedYear, orderedMonth)).getTime() / 1000;
        if (!acc[orderKey]) {
        ...
        if (
          isLastDayOfMonth(dateOrdered) &&
          isNextCalendarMonth(dateOrdered, dateSent)
        ) {
          const sentKey =
            new Date(Date.UTC(sentYear, sentMonth)).getTime() / 1000;
          if (!acc[sentKey]) {
  ...
  function findCategoriesByPeriod(
    orders,
    ...
    filteredOrders.forEach((order) => {
      order.rows.forEach((item) => {
        unsortedCategories[item.product.categoryTree] =
          (unsortedCategories[item.product.categoryTree] || 0) + 1;
      });
    });
  ...
  function findStreaks(orders, minimumSum = 500) {
    ...
    for (let i = 0; i < orderDates.length; i++) {
      ...
      if (currentDate < cheevoStartDate || orderDates[i].totalSum < minimumSum)
        continue;
  ...
  async function _clearAndAddStatistics(event) {
    ...
    let paths = [
      "/html/body/div[2]/div[2]/div[1]/div[3]/main/div/div[4]/div[1]/div/article/div[2]/section",
    ...
    image.setAttribute(
      "href",
      "https://cdn.webhallen.com/img/loading_light.svg"
    );
    ...
    if (orders) {
      ...
      if (experience) {
        injectPath.appendChild(
          addDataToDiv("Experience", generateExperienceTable(experience))
        );
      }
      ...
      if (streaks) {
        injectPath.appendChild(
          addDataToDiv("Streaks", generateStreaksTable(streaks))
        );
      }
      ...
      if (hoarder) {
        injectPath.appendChild(
          addDataToDiv("Hoarder Top 10", generateHoarderTable(hoarder))
        );
      }
      ...
      if (categories) {
        injectPath.appendChild(
          addDataToDiv("Kategorier", generateCategoriesTable(categories))
        );
      }
      ...
      if (orderMonths) {
        injectPath.appendChild(
          addDataToDiv("Ordrar per månad", generateMonthsTable(orderMonths))
        );
      }
    }
  }
  ...
  function addLink() {
    ...
    var ul = document.evaluate(
      "/html/body/div[2]/div[2]/div[1]/div[3]/main/div/div[4]/div[1]/div/article/div[2]/div/nav/div[1]/div[1]/ul",
      document,
      null,
      XPathResult.FIRST_ORDERED_NODE_TYPE,
      null
